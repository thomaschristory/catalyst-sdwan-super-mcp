...
The `docs` workflow builds the site with [Zensical](https://zensical.org/) and deploys it to GitHub Pages on pushes to `main` that touch `docs/**`, `mkdocs.yml`, or the workflow itself (and via `workflow_dispatch`). Zensical reads `mkdocs.yml` natively — that is still the config file.
