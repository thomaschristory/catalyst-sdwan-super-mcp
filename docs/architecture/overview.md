...
| `dispatcher.py` | One `httpx2.AsyncClient` per server. Looks ops up by `action_name`, routes params (path / query / body), executes, applies the [retry + timeout policy](../reference/configuration.md#retry-behavior), and retries once on session expiry. |
