...
- **Two auth modes to vManage** — modern JWT (20.18.1+) and legacy session (older).
- **Three transports** — stdio (Claude Desktop), SSE, streamable-HTTP. HTTP transports ship with first-class bearer-token auth and auto-demote non-loopback binds to `127.0.0.1` when unauthenticated. [HTTP auth](guides/mcp-clients.md).
- **Response pagination** — auto-follow for scroll- and offset-style bulk endpoints, with resumable cursors. [Pagination](guides/pagination.md).
- **Configurable retry + timeout** — exponential-backoff retry of transient 5xx and connection errors. [Configuration](reference/configuration.md).
- **Auto-fetch specs** — bump `active_version`, the loader pulls and stitches the matching OpenAPI spec from `developer.cisco.com` on startup. [Spec versions](guides/spec-versions.md).
